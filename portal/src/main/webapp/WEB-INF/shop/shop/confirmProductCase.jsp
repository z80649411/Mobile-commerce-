...
<div class="w ofc case">
					...
					<c:forEach items="${userAddress}" var="addr">
						<tr>
							<td><input type="radio"
									   ...
									   value="${addr.shipAddrId}" name="address" />${addr.province} &nbsp;${addr.city} &nbsp;${addr.district}&nbsp; ${addr.shipName}&nbsp;${addr.zipCode} </td>
						</tr>
					</c:forEach>
				<tr>
	...
	<div class="box bg_white pb">
		...
		<c:forEach items="${carts}" var="cart">
			...
			<td class="img48x20">
				<span class="inb"><img src="${file_path }${cart.sku.item.imgs }" /></span>
				<a href="#" title="1381050188" class="b f20">${cart.sku.item.itemName }</a>
			</td>
			<td>￥${cart.sku.skuPrice }</td>
			<td>${cart.quantity }</td>
			<td class="orange">￥${cart.sku.skuPrice * cart.quantity }</td>
			</tr>
		</c:forEach>
		...
		<div class="ofc">
			...
			<div class="r">
				...
				<dl class="total">
					...
					<dd class="orange"><em class="l">应付总额：</em>￥<var id="totalMoney" name=""><fmt:formatNumber value="${totalPrice }" pattern="#0.00"/></var></dd>
					<dd class="alg_c">
						<input id="submitOrderID" type="submit" value="订单提交！" class="hand btn136x36a" "/></dd>
				</dl>
			</div>
		</div>
